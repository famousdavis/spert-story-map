...
export default function useSizingDrag({ layout, zoom, pan, updateProduct }: { layout: SizingLayout; zoom: number; pan: { x: number; y: number }; updateProduct: UpdateProduct }) {
  ...
  const screenToMap = useCallback((screenX: number, screenY: number, containerRect: DOMRect) => ({
  ...
  const findTargetSize = useCallback((mapX: number, mapY: number): string | null | undefined => {
